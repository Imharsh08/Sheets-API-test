...
const PendingTable: React.FC<Props> = ({ data }) => {
  const sorted = [...data].sort((a, b) => (b.activityDate || '').localeCompare(a.activityDate || ''));
  ...
  return (
    <div className="flex-1 overflow-auto">
      <table className="w-full border-collapse min-w-[1100px]">
        <thead>
          <tr>
            <th colSpan={4} className="text-left py-1 px-3 text-[8px] font-semibold tracking-[1px] uppercase"
            ...
            <th colSpan={2} className="text-left py-1 px-3 text-[8px] font-semibold tracking-[1px] uppercase"
                style={{ background: 'var(--color-ink)', color: '#f0a080', position: 'sticky', top: 0, zIndex: 10 }}>Defects</th>
            <th colSpan={2} className="text-left py-1 px-3 text-[8px] font-semibold tracking-[1px] uppercase"
                style={{ background: 'var(--color-ink)', color: '#f0c080', position: 'sticky', top: 0, zIndex: 10 }}>Dispatch</th>
            <th colSpan={1} className="text-left py-1 px-3 text-[8px] font-semibold tracking-[1px] uppercase"
                style={{ background: 'var(--color-ink)', color: '#94a3b8', position: 'sticky', top: 0, zIndex: 10 }}>Last Activity</th>
          </tr>
          <tr>
            {['Die','Job Card No.','Item','Order Qty'].map(h =>
            ...
            {['Done','Pending'].map(h =>
              <th key={'m'+h} className="text-left py-1.5 px-3 text-[9px] font-semibold tracking-[1px] uppercase whitespace-nowrap border-b-2 bg-mol"
            ...
            {['Done','Pending'].map(h =>
              <th key={'f'+h} className="text-left py-1.5 px-3 text-[9px] font-semibold tracking-[1px] uppercase whitespace-nowrap border-b-2 bg-fin"
            ...
            {['Passed','Pending'].map(h =>
              <th key={'i'+h} className="text-left py-1.5 px-3 text-[9px] font-semibold tracking-[1px] uppercase whitespace-nowrap border-b-2 bg-ins"
                  style={{ background: 'rgba(26,107,74,0.04)', color: 'var(--color-txd)', borderColor: 'var(--color-bdr2)', position: 'sticky', top: 24, zIndex: 10 }}>{h}</th>
            )}
            {['Rework','Rejected'].map(h =>
              <th key={'d'+h} className="text-left py-1.5 px-3 text-[9px] font-semibold tracking-[1px] uppercase whitespace-nowrap border-b-2 bg-def"
                  style={{ background: 'rgba(185,28,28,0.04)', color: 'var(--color-txd)', borderColor: 'var(--color-bdr2)', position: 'sticky', top: 24, zIndex: 10 }}>{h}</th>
            )}
            {['Done','Pending'].map(h =>
              <th key={'ds'+h} className="text-left py-1.5 px-3 text-[9px] font-semibold tracking-[1px] uppercase whitespace-nowrap border-b-2 bg-dis"
            ...
            <th className="text-left py-1.5 px-3 text-[9px] font-semibold tracking-[1px] uppercase whitespace-nowrap border-b-2"
                style={{ background: 'var(--color-sur2)', color: 'var(--color-txd)', borderColor: 'var(--color-bdr2)', position: 'sticky', top: 24, zIndex: 10 }}>Date</th>
          </tr>
        </thead>
        <tbody>
          {sorted.length === 0 ? (
            <tr><td colSpan={15} className="text-center py-12 italic text-[12px]" style={{ color: 'var(--color-txd)' }}>No orders match your filters.</td></tr>
          ) : sorted.map(r => {
            ...
            return (
              <tr key={r.jc} className="border-b hover:bg-[#f0efea] transition-colors" style={{ borderColor: 'var(--color-bdr)' }}>
                ...
                <td className="py-2 px-3 text-[10px] whitespace-nowrap" style={{ color: 'var(--color-txd)' }}>{r.activityDate || '—'}</td>
              </tr>
            );
          })}
        </tbody>
